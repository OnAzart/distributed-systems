...
def receive_message():
    ...
    if purpose == 'restore':
        reserve_queue = message_to_append
        logging.info('Reserve queue has been restored.')
    elif purpose == 'append':
...
def welcome():
    if request.args.get('action', None) == 'List':
        return render_template('secondary.html', list_messages=[message[1] for message in reserve_queue])
